# Kernel Based Regression (KBR), uncorrected
#TODO: Change variable N to W (weights)

function KBRu_moments_A(X, tau_i_range, x_range, h, kernel::Kernel)
    n = length(X)
...
function KBRu_moments_A2(X, tau_i_range, x_range, h, kernel)
    n = length(X)
    nτ = length(tau_i_range)
    nx = length(x_range)
    N = zeros(nτ,nx)
...
## Modulo moments
function KBRu_moments_mod(X, tau_i_range, x_range, h, kernel, period)
    n = length(X)
    nτ = length(tau_i_range)
    nx = length(x_range)
    N = zeros(nτ,nx)
