...
## Single step algorithm, 1D (for testing), uncorrected

mutable struct OKBRu_single{T<:AbstractRange,K<:Kernel}
    x_eval_points::T
    w::Array{Float64,1}
    M1::Array{Float64,1}
    M2::Array{Float64,1}
...
function OKBRu_single(x_eval_points, kernel, h::Float64)
...
function add_data!(okbr::OKBRu_single, X_right)
    X_left = okbr.mem
    if !isnan(X_left)
        ΔX = X_right - X_left
...
mutable struct OKBRu_multiple{T<:AbstractRange,K<:Kernel}
    x_eval_points::T
    tau_i::UnitRange{Int}
    w::Array{Float64,2}
    M1::Array{Float64,2}
...
function OKBRu_multiple(x_eval_points, tau_i, kernel, h::Float64)
...
# Scaled moments
M1τ(okbr::OKBRu_multiple, dt) = okbr.M1 ./ (dt*okbr.tau_i)
M2τ(okbr::OKBRu_multiple, dt) = okbr.M2 ./ (dt*okbr.tau_i)

# Add data to moments
function add_data!(okbr::OKBRu_multiple, X_right)
