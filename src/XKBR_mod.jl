...
## OHBR modulo moments, single step algorithm, 1D (for testing)
...
mutable struct OKBR_mod_single{T<:AbstractRange,K<:Kernel}
    x_eval_points::T
    period::Float64
    w::Array{Float64,1}
    M1::Array{Float64,1}
...
function OKBR_mod_single(x_eval_points, period, kernel, h::Float64)
...
# Add data
function add_data!(okbr::OKBR_mod_single, X_right)
    X_left = okbr.mem
    if !isnan(X_left)
        ΔX = X_right - X_left
        for (j_ind, x_eval) in enumerate(okbr.x_eval_points)
            ...
            if K_weight > 0.0
                ...
                setindex!(
                    okbr.M2,
                    OnlineMoments.update_wvar(okbr.M2[j_ind], M1_old, w_old, ΔX, okbr.M1[j_ind], K_weight),
                    j_ind
                )
            end
        end
...
#TODO: generalize tau_i
mutable struct OKBR_mod_multiple{T<:AbstractRange,K<:Kernel}
    x_eval_points::T
    tau_i::UnitRange{Int}
    period::Float64
    w::Array{Int64,2}
...
function OKBR_mod_multiple(x_eval_points, tau_i, period, kernel, h::Float64)
    Nx = length(x_eval_points)
    τ_len = length(tau_i)
    mem = zeros(Float64, τ_len)
    mem .= NaN
...
# Scaled moments
M1τ(okbr::OKBR_mod_multiple, dt) = okbr.M1 ./ (dt*okbr.tau_i)
M2τ(okbr::OKBR_mod_multiple, dt) = okbr.M2 ./ (dt*okbr.tau_i)

# Add data to moments
function add_data!(okbr::OKBR_mod_multiple, X_right)
